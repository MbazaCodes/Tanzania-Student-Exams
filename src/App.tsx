...
import { Library, Upload, FilePlus2, ClipboardList, PenSquare, CheckCircle2, BarChart3, ShieldCheck, GraduationCap, Menu, X, CalendarClock, Bell, Wifi } from 'lucide-react'
import { Toaster, toast } from 'sonner'
import { getCurrentUser, getAllUsers, setSessionUid } from '@/lib/api'
import type { User } from '@/lib/types'
import { ROLE_LABEL } from '@/lib/types'
import { useStore, type TabId } from '@/lib/store'
import { Badge, Button, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/index'
...
const ROLE_COLOR: Record<string,string> = { student:'bg-emerald-50 text-emerald-700 border-emerald-200', teacher:'bg-amber-50 text-amber-700 border-amber-200', school_admin:'bg-sky-50 text-sky-700 border-sky-200', super_admin:'bg-purple-50 text-purple-700 border-purple-200' }
...
export default function App() {
  ...
  const { tab, setTab, nonce } = useStore()

  const loadMe = useCallback(async () => {
    try { const [u, all] = await Promise.all([getCurrentUser(), getAllUsers()]); setUser(u); setSwitchers(all as any) }
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-hero text-white shadow-lg">
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4">
          ...
          <div className="ml-auto flex items-center gap-2">
            {isIndependent&&<span className="hidden sm:inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium border border-amber-400/40 bg-amber-500/20 text-amber-200"><Wifi className="h-3 w-3"/>Independent</span>}
            <button onClick={()=>setTab('schedule')} className="hidden sm:inline-flex items-center gap-1 rounded-md px-3 py-1.5 text-sm hover:bg-white/10 text-white relative">
              <Bell className="h-4 w-4"/><span className="hidden lg:inline ml-1">Alerts</span>
              <span className="absolute -top-0.5 -right-0.5 flex h-2 w-2 rounded-full ring-2 ring-blue-900" style={{background:'var(--gold)'}}/>
            </button>
            {user&&<span className={cn('hidden sm:inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium border', ROLE_COLOR[role])}>{ROLE_LABEL[role]}</span>}
            <Select value={user?.id??''} onValueChange={handleSwitch}>
              <SelectTrigger className="w-[160px] sm:w-[200px] bg-white/10 border-white/20 text-white hover:bg-white/15 text-sm h-8">
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-1">
        <aside className="hidden md:flex w-60 shrink-0 flex-col gap-1 border-r border-border bg-card p-3">
          {visibleNav.map(n=>(
            <button key={n.id} onClick={()=>setTab(n.id)} className={cn('flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors', activeTab===n.id ? 'text-white shadow-sm' : 'text-muted-foreground hover:text-foreground')} style={activeTab===n.id ? {background:'var(--navy)'} : {}}>
              <n.icon className={cn('h-4 w-4')}/>
              {n.label}
            </button>
          ))}
          <div className="mt-auto rounded-lg border p-3 text-xs text-muted-foreground" style={{borderColor:'rgba(0,166,81,0.2)',background:'rgba(0,166,81,0.05)'}}>
            <p className="font-medium" style={{color:'var(--navy)'}}>Role-based access</p>
            <p className="mt-1">Switch users to see student, teacher, school admin and super admin views.</p>
          </div>
        </aside>
        ...
        {mobileOpen&&(
          <div className="md:hidden fixed inset-0 z-50">
            ...
            <div className="absolute left-0 top-0 h-full w-64 bg-card p-3 shadow-xl flex flex-col gap-1 overflow-y-auto">
              <div className="flex items-center justify-between px-2 py-2"><span className="font-semibold">Navigation</span><button onClick={()=>setMobileOpen(false)} className="rounded-md p-1 hover:bg-accent"><X className="h-4 w-4"/></button></div>
              {visibleNav.map(n=>(
                <button key={n.id} onClick={()=>{setTab(n.id);setMobileOpen(false)}} className={cn('flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors', activeTab===n.id?'text-white':'text-muted-foreground hover:bg-accent')} style={activeTab===n.id?{background:'var(--navy)'}:{}}>
